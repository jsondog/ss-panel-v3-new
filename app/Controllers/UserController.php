...
class UserController extends BaseController
{
    ...
    public function doCheckIn($request, $response, $args)
    {
        ...
        if ($this->user->class == 0){
            $this->user->class_expire = date("Y-m-d H:i:s", time()+Config::get('checkinTime')*3600);
            $this->user->class = Config::get('user_class_default');
            $this->user->last_check_in_time = time();
            $this->user->save();
            $res['msg'] = sprintf("获得了 %u MB流量.您可以使用VIP节点了", $traffic);
            $res['ret'] = 1;
        }else{
            $this->user->last_check_in_time = time();
            $this->user->save();
