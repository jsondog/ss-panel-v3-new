            ...
            switch ($command) {
                ...
                case 'checkin':
                    if (!$user->isAbleToCheckin()) {
                        $bot->sendMessage($message->getChat()->getId(), "你已经签到过了，请明天再来吧！", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
                        break;
                    }
                    ...
                    $user->save();
                    $bot->sendMessage($message->getChat()->getId(), " 签到成功！恭喜你获得了 ".$traffic." MB 流量！", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
                    break;
                default:
                    $bot->sendMessage($message->getChat()->getId(), "???", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
            }
        } else {
            $bot->sendMessage($message->getChat()->getId(), "您未绑定本站账号，您可以登录https://www.sockspeed.com 中点击 资料编辑 ，滑到页面最下方，就可以看到 Telegram 绑定指示二维码了，绑定您的账号", $parseMode = null, $disablePreview = false, $replyToMessageId = $reply_to);
        }
    }
...
//                    break;
                ...
                case 'help':
                    $help_list = "命令列表：
						...
						您可以登录https://www.sockspeed.com 中点击 资料编辑 ，滑到页面最下方，就可以看到 Telegram 绑定指示二维码了，绑定您的账号，更多精彩功能等着您去发掘。
					";
            ...
            switch ($command) {
                ...
                case 'help':
                    $help_list_group = "命令列表：
						...
						您可以登录https://www.sockspeed.com 中点击 资料编辑 ，滑到页面最下方，就可以看到 Telegram 绑定二维码指示了，绑定您的账号，更多精彩功能等着您去发掘。
					";
