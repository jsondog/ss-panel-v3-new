...
class XCat
{
    ...
    public function boot()
    {
        switch ($this->argv[1]) {
            case("install"):
                return $this->install(); //安装系统
            case("createAdmin"):
                return $this->createAdmin(); //创建管理员
            case("resetTraffic"):
                return $this->resetTraffic(); //重置流量
            case("setTelegram"):
                return $this->setTelegram(); //设置setTelegram
            case("initQQWry"):
                return $this->initQQWry(); //重置QQ登录
            case("sendDiaryMail"):
                return DailyMail::sendDailyMail(); //发送每日流量通知邮件
            case("reall"):
                return DailyMail::reall();
            case("syncusers"):
                return SyncRadius::syncusers(); //VPN的
            case("synclogin"):
                return SyncRadius::synclogin(); //VPN的
            case("syncvpn"):
                return SyncRadius::syncvpn(); //VPN的
            case("nousers"):
                return ExtMail::sendNoMail(); //发送给没有使用过的用户邮件通知
            case("oldusers"):
                return ExtMail::sendOldMail(); //发送给以前使用过最近没有使用的用户邮件通知
            case("syncnode"):
                return Job::syncnode();
            case("syncnasnode"):
