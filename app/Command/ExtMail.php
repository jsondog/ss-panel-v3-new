...
class ExtMail
{
    ...
    public static function sendOldMail()
    {
        $users = User::all();
        foreach ($users as $user) {
            if ($user->t!=0&&$user->t<1728000) {
                echo "Send daily mail to user: ".$user->id;
                $subject = Config::get('appName')."-期待您的回归";
                $to = $user->email;
                $text = "似乎您在 20多天以来就没有使用过".Config::get('appName')."了呢，如果您在使用上遇到了任何困难，请不要犹豫，登录到".Config::get('appName')."，您就会知道如何使用了，特别是对于 iOS 用户，最近在使用的优化上大家都付出了很多的努力。期待您的回归～" ;
                try {
                    Mail::send($to, $subject, 'ext/back.tpl', [
                        "user" => $user,"text" => $text
                    ], [
